...
public class AccessLog extends Configuration implements Tool {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 파라미터는 변환할 원본 파일(폴더)과 시퀸스파일로 변환될 파일(폴더)로 2개를 받음
        if (args.length != 2) {

            log.info("변환할 원본 파일(폴더)과 시퀸스파일로 변환될 파일(폴더)를 입력해야 합니다.");
            System.exit(-1);
    ...
    public int run(String[] args) throws Exception {
        ...
        // 맵리듀스 잡 이름 설정, 리소스 매니저 등 맵리듀스 실행 결과 밎 로그 확인할 때 편리함
...
//        // Mapper에서 전달하는 데이터타입과 동일한 경우, job.setMapOutputKeyClass(Text.class); 생략 가능
//        job.setOutputKeyClass(Text.class);
//
//        // 분석 결과가 저장될때 사용될 키의 데이터 타입
//        // Mapper에서 전달하는 데이터타입과 동일한 경우, job.setOutputValueClass(Text.class); 생략 가능
