...
public class MonthLog2Mapper extends Mapper<LongWritable, Text, Text, Text> {
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        if (fields[3].length() > 2) { // 일부 데이터가 요청일 시 누락된 경우가 있어 요청일시 값이 존재하는지 체크
        ...
        if (fields[5].length() > 2) { // 일부 데이터가 요청일 시 누락된 경우가 있어 요청일시 값이 존재하는지 체크

            reqMethod = fields[5].substring(1); // 요청일시 추출

        }

        String reqURI = fields[6]; // 요청 URI 추출출
